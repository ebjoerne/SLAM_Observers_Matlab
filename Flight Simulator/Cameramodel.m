...
%     R_bc=[0 0 -1.0000;0 1 0; 1 0 0];
    Xmin=Position(1,1)-(-Position(3,1))*1;
    Xmax=Position(1,1)+(-Position(3,1))*1;
    Ymin=Position(2,1)-(-Position(3,1))*1;
    Ymax=Position(2,1)+(-Position(3,1))*1;
    n=length(PP);
...
%     Index1 = intersect(IX,IY);
    ...
    for i=1:n;
         LOS_n=(PP(Index1(i),:)'-Position)+10^-7*randn(3,1)*0;
         LOS_n=LOS_n/norm(PP(Index1(i),:)-Position');
         LOS_b=R_nb'*LOS_n;
