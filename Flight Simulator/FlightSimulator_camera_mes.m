...
floorAndSealing=0;
comp_filter=0;
VO=1;  % VO algorithm chosen for extracting check velocity and range 
plane=0; % Points are on plane or not
...
%Creating map
N=3000;
PP=[-25+50*rand(N,1), -25+50*rand(N,1) ,-4*(rand(N,1) < 0.5)*floorAndSealing+0.3*(rand(N,1) < 0.5)*(1-plane)];

Convergance_iterations=0; % How many iterations before we are confident on convergance
...
%         drawnow
        ...
        if (VO==1)
            ...
            [relativeOrientation,relativeLocation,validPointsFraction] = relativeCameraPose(E,cameraParams,matchedPoints1,matchedPoints2);
            if (validPointsFraction>0.95)
            %             relativeLocation=v_b'*h;
            relativeLocation=relativeLocation/norm(relativeLocation)*0.01;

           
%             relativeOrientation
%             relativeOrientation_true=eye(3)+R_nb_dot*h; [U, ~, V]=svd(relativeOrientation_true); relativeOrientation_true=U*V'
            else
               K= cameraParams.IntrinsicMatrix;
               H = homography_solve(matchedPoints1',matchedPoints2');
               H_rt =  K\H;
               R_hat=[H_rt(:,1) H_rt(:,2) S(H_rt(:,1))*H_rt(:,2)];
               [U, LAMBDA, V]=svd(R_hat); relativeOrientation=U*V';
               H_t=H_rt-relativeOrientation*LAMBDA(1,1);
               
               relativeLocation=(H_t*[0 0 -1]')';
        ...
        elseif (VO==2)
            ...
            relativeLocation=(H_t*[0 0 -1]')';   
            validPointsFraction=0;
        else
            v_check=v_b;
            rho_check=rho_measurments;
        end
        ...
        test=cam_estimates./real_estimates
%       v_b'
        v_b'/norm(v_b)
        relativeLocation./norm(relativeLocation)
        validPointsFraction
        rho_check=rho_measurments_cam;
        v_check=relativeLocation./h
        rho_measurments(index_vel)=rho_check;
...
%         g_hat
    ...
    for j=Index_M'
        l=l+1;
        if (norm(L_HAT(j,:))==0 || L_HAT(j,:)*LOS_mesurments(l,:)'<0.97*0||Converge_Counter(j)>Convergance_iterations-3)
            L_HAT(j,:)=LOS_mesurments(l,:);
            D_HAT(j)=mean(D_HAT(Index_M))*0+0.1;
            D_HAT(j)=1/(rho_measurments(l)*10);
